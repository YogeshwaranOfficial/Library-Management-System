...
export const UserModal: React.FC<UserModalProps> = ({ isOpen, onClose, initialData }) => {
  ...
  const [password, setPassword] = useState(() => isEditMode ? "••••••••" : "");
  const [confirmPassword, setConfirmPassword] = useState(() => isEditMode ? "••••••••" : "");
  ...
  const userMutation = useMutation({
    mutationFn: async (payload: Record<string, string>) => {
      if (isEditMode && initialData) {
        const response = await axiosClient.put(`/admin/edit-user/${initialData.user_id}`, payload);
    ...
    onError: (error: AxiosError<BackendErrorResponse>) => {
      ...
      toast.error(serverMessage || "Failed to finalize account registry context changes.");
    },
  });
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-gray-200">
        ...
        <form onSubmit={handleSubmission} className="p-6 space-y-5 text-[#2D3748]">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-2.5 border rounded-xl text-xs font-mono tracking-wide font-semibold outline-hidden transition-all focus:ring-0 ${
                    isEditMode 
          ...
          <div className="pt-4 flex justify-end gap-3 border-t border-gray-100 text-xs font-bold tracking-wide">
            ...
            >
              ✓ {userMutation.isPending ? "Syncing..." : isEditMode ? "Commit Edit" : "Create Operator"}
            </button>
          </div>
        </form>
      </div>
