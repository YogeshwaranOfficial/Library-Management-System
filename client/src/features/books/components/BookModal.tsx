    ...
    <div className="fixed inset-0 bg-ocean-blue/50 backdrop-blur-xs flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-gray-100 animate-zoom-in">
        <div className="bg-linear-to-r from-ocean-light to-ocean-blue p-5 text-white flex justify-between items-center">
          <h3 className="font-bold text-lg">{editingBook ? "Modify Cataloged Asset Details" : "Register New Media Asset"}</h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          <div>
            <label className="text-xs font-bold text-gray-700 uppercase tracking-wide block mb-1">Book Title Identification Statement</label>
            <input type="text" {...register("title")} placeholder="e.g., Introduction to Relational Databases" className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-hidden focus:bg-white focus:ring-2 focus:ring-teal-100 focus:border-teal-brand" />
            {errors.title && <p className="text-xs text-red-500 mt-1 font-medium">{errors.title.message}</p>}
          </div>

          <div>
            <label className="text-xs font-bold text-gray-700 uppercase tracking-wide block mb-1">Author / Creator Source Reference</label>
            <input type="text" {...register("author")} placeholder="e.g., C.J. Date" className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-hidden focus:bg-white focus:ring-2 focus:ring-teal-100 focus:border-teal-brand" />
            {errors.author && <p className="text-xs text-red-500 mt-1 font-medium">{errors.author.message}</p>}
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-gray-700 uppercase tracking-wide block mb-1">Total System Stock Volumes</label>
            ...
            <div>
              <label className="text-xs font-bold text-gray-700 uppercase tracking-wide block mb-1">Asset Classification Category</label>
          ...
          {editingBook && (
            <div className="grid grid-cols-2 gap-4 p-3 bg-amber-50/60 border border-amber-100 rounded-xl">
              <div>
                <span className="text-[11px] font-bold text-amber-800 uppercase block">Available on Shelves</span>
                <div className="text-lg font-bold text-amber-900 mt-0.5">{editingBook.availableCopies} Copies</div>
                <span className="text-[10px] text-amber-600 block leading-tight mt-1">Read-Only: Handled by system transactions.</span>
              </div>
              <div>
                <span className="text-[11px] font-bold text-amber-800 uppercase block">Active Circulation Count</span>
                <div className="text-lg font-bold text-amber-900 mt-0.5">{editingBook.lendingCount} Loans</div>
                <span className="text-[10px] text-amber-600 block leading-tight mt-1">Updates live when books are issued or returned.</span>
              </div>
            </div>
          )}
