...
export const ManageCategories = () => {
  ...
  >("NONE");
  const [borrowSort, setBorrowSort] = useState<
    "NONE" | "HIGH_TO_LOW" | "LOW_TO_HIGH"
  >("NONE");
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<CategoryMetrics | null>(null);
  const [isDetailsOpen, setIsDetailsOpen] = useState(false);

  // 🚨 Deletion Reconfirmation Setup State
  const [deleteConfirmConfig, setDeleteConfirmConfig] = useState<{
    isOpen: boolean;
    targetCat: CategoryMetrics | null;
  }>({
    isOpen: false,
    targetCat: null,
  });
  ...
  const { data: serverPayload, isLoading } = useQuery<ServerPaginationWrapper>({
    ...
    queryFn: async () => {
      ...
      const res = await axiosClient.get(
        `/categories/metrics?${params.toString()}`,
      );
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["libraryCategoriesAggregationFeed"],
      });
    ...
    onError: () =>
      toast.error("Server baseline engine rejected insertion pipeline."),
  });

  // Mutation: Save Partial Update Edits
  const updateMutation = useMutation({
    mutationFn: async ({ id, name }: { id: string; name: string }) => {
      return await axiosClient.patch(`/categories/${id}`, {
        category_name: name,
      });
    },
    onSuccess: (res) => {
        ...
        setSelectedCategory((prev) =>
          prev
            ? {
                ...prev,
                category_name:
                  responseData?.data?.category_name || prev.category_name,
              }
            : null,
        );
      }
    ...
    onError: () =>
      toast.error("Failed to alter category reference values safely."),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(
        "Categories deleted successfully and Books under the category also removed.",
      );
    ...
    onError: () =>
      toast.error("Database constraint rejected Delete transaction profile."),
  });
  ...
  const handleExecuteCreate = async (e: React.FormEvent) => {
    ...
    const validator = categoryFormSchema(activeNames).safeParse({
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-card-bg p-5 mb-5 rounded-2xl border border-border-main shadow-xs shrink-0">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5 font-medium">
            Control library book categories, analyze category engagement
            volumes, and manage inventory segments.
          </p>
        </div>
        <button
          type="button"
          ...
          <div className="w-full md:w-48 relative">
            <select
              ...
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => {
                setBookSort(
                  e.target.value as "NONE" | "HIGH_TO_LOW" | "LOW_TO_HIGH",
                );
                setBorrowSort("NONE");
                setCurrentPage(1);
              }}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col flex-1 space-y-5">
          <div className="bg-card-bg rounded-2xl border border-border-main shadow-xs overflow-hidden">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-border-main text-[11px] font-bold text-slate-400 uppercase bg-slate-50 tracking-wider">
                    ...
                    <th className="py-3.5 px-5">
                      Active Circulation Borrow Count
                    </th>
                  </tr>
                </thead>
                  ...
                  {categoriesList.length === 0 ? (
                    ...
                    categoriesList.map((cat: CategoryMetrics) => (
                      ...
                      >
                        ...
                        <td className="py-3.5 px-5 text-slate-500 font-medium font-mono text-xs">
                          {cat.booksCount} books
                        </td>
                        <td className="py-3.5 px-5">
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold tracking-wide uppercase border bg-slate-50 text-slate-600 border-slate-100">
            ...
            {/* 🏁 Standard Synchronized Footer Pagination Engine Controls */}
            {totalPages > 0 && (
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <form
            ...
            className="relative z-10 w-full max-w-md bg-card-bg rounded-2xl p-6 border border-border-main shadow-2xl space-y-5 text-text-main"
          >
            <div>
              <h3 className="text-lg font-bold text-text-main tracking-tight flex items-center gap-2">
                <FolderPlus size={18} className="text-slate-500" /> Add Category
                Classification
              </h3>
              <p className="text-xs text-slate-500 mt-1 font-medium">
                Input a unique classification name to initialize shelf tags
                mapping slots.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full px-4 py-2 bg-slate-50 border border-border-main text-text-main rounded-xl text-xs sm:text-sm font-bold focus:bg-card-bg outline-hidden focus:ring-4 focus:ring-slate-900/5 focus:border-slate-900 transition-all placeholder-slate-400"
              />
            ...
            <div className="flex justify-end gap-3 pt-2 text-xs font-bold uppercase tracking-wider">
              <button
                type="button"
                onClick={() => setIsCreateOpen(false)}
                className="px-4 py-2 border border-border-main text-slate-500 rounded-xl hover:text-text-main hover:border-slate-300 transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
