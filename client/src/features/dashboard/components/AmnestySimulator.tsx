...
export const AmnestySimulator = ({ totalOutstanding, discount, onChange }: SimulatorProps) => {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-xs space-y-4 text-left font-sans">
      <div>
        <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
          <Sparkles size={12} className="text-amber-500" /> Bulk Amnesty Clearance Simulator
        </h3>
      ...
      <div className="p-4 bg-amber-50/40 border border-amber-100 rounded-xl space-y-3">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1.5 text-xs font-bold">
          <span className="text-slate-700 tracking-wide">Relief Rate: {discount}%</span>
          <span className="text-amber-700 font-mono">Projected Cash Collection: ₹{projectedRecovery}</span>
        </div>
      ...
      <div className="text-[11px] text-slate-400 font-semibold italic leading-relaxed">
        Offering this clearance drops <span className="text-rose-600 not-italic font-bold">₹{waivedFine}</span> in toxic debt and creates an immediate collection incentive for unreturned books.
      </div>
    </div>
  );
};