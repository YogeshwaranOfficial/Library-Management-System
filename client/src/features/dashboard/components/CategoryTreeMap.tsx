...
export const CategoryTreeMap = ({
  ...
  return (
    <div className="bg-card-bg p-6 rounded-2xl border border-border-main/60 shadow-xs space-y-4 h-full flex flex-col justify-between text-left font-sans text-text-main">
      ...
      <div className="flex w-full rounded-xl overflow-hidden h-14 border border-border-main/40 shadow-inner my-2 shrink-0 bg-page-bg">
        {categories.map((cat) => (
          <div
            key={cat.name}
            className={`${cat.color} h-full transition-all duration-200 group relative cursor-pointer border-r border-page-bg/40 last:border-0`}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
        {categories.map((cat) => (
          ...
          >
            <span
              className={`w-2.5 h-2.5 rounded-full ${cat.color} shrink-0 ring-4 ring-card-bg`}
            />
