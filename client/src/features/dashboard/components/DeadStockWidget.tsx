    ...
    <div className="space-y-2.5 my-4 overflow-y-auto max-h-35 pr-1 flex-1">
      {items.length === 0 ? (
        ...
        items.map((item) => (
          <div
            key={item.id}
            className="flex justify-between items-center bg-slate-50/50 border border-border-main/60 p-2.5 rounded-xl transition-colors"
          >
            <span
              className="text-xs font-bold text-slate-800 truncate max-w-35 sm:max-w-42.5"
              title={item.title}
            >
              {item.title}
            </span>
