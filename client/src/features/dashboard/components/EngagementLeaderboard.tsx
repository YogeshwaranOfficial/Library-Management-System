...
export const EngagementLeaderboard = ({ members }: { members: TopUser[] }) => (
  <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-xs space-y-4 h-full flex flex-col justify-between text-left font-sans">
      ...
      {members.slice(0, 3).map((member, index) => (
        <div key={member.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
          <div className="flex items-center gap-2.5">
            <span className="text-[11px] font-mono font-bold text-slate-900 bg-amber-50 border border-amber-100 h-5 w-6 rounded-md flex items-center justify-center">
              #{index + 1}
            </span>
            <span className="text-xs font-bold text-slate-800 truncate max-w-27.5 sm:max-w-35" title={member.name}>
              {member.name}
            </span>
          </div>
          <div className="text-right">
            <p className="text-xs font-bold text-slate-900 font-mono">{member.loans} Loans</p>
    ...
    {/* <div className="bg-slate-50 border border-slate-200/60 text-slate-600 text-[10px] font-bold p-2.5 rounded-xl text-center uppercase tracking-wider">
