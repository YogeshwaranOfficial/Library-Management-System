...
const MetricCard = ({
  ...
  <div className="bg-white p-5 rounded-2xl border border-border-main/60 shadow-xs flex justify-between items-start transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 text-left font-sans">
    <div className="space-y-1.5">
      <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wide block">
        {title}
      </span>
      <div className="text-2xl font-bold text-text-main tracking-tight font-sans">
        {value}
      </div>
      {subtext && (
    ...
    <div
      className={`p-3 rounded-xl ${bgClass} shadow-xs shrink-0 flex items-center justify-center`}
    >
      {icon}
    </div>
  </div>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
      <MetricCard
        ...
        bgClass="bg-amber-50/50 text-text-main border border-amber-100"
        icon={<BookOpen size={20} className="text-text-main" />}
      />
      <MetricCard
      ...
      <MetricCard
        ...
        bgClass="bg-slate-100 text-slate-800 border border-border-main/60"
        icon={<Users size={20} />}
      />
      <MetricCard
        title="Books Overdue"
