    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-xs space-y-4 text-left font-sans">
      <div>
        <h3 className="text-[11px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
          <TrendingUp size={14} className="text-slate-500" /> Foot-Traffic Velocity
        </h3>
      ...
      <div className="flex items-end justify-between h-40 pt-6 px-2 bg-slate-50/40 border border-slate-100 rounded-xl">
        {data.map((item) => (
          <div key={item.day} className="flex flex-col items-center gap-2 group w-full">
            ...
            <div className="text-[10px] font-mono font-bold text-slate-900 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-sm opacity-0 group-hover:opacity-100 transition-all duration-200 transform translate-y-1 group-hover:-translate-y-0.5 pointer-events-none">
              {item.count}
            </div>
