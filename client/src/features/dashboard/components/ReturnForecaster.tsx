    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-xs space-y-4 text-left font-sans">
      <div>
        <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
          <CalendarDays size={14} className="text-slate-500" /> 7-Day Return Flow Forecaster
        </h3>
      ...
      <div className="space-y-3 pt-1">
        {forecast.map((day) => (
          <div key={day.date} className="flex items-center gap-4 text-xs font-semibold">
            <span className="w-16 text-slate-500 font-bold tracking-wide">{day.date}</span>
            ...
            <span className="w-16 text-right font-mono font-bold text-slate-900 shrink-0">
              {day.count} items
            </span>
          </div>
        ))}
