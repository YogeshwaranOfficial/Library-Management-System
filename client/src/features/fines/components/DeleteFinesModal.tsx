...
export const DeleteFinesModal = ({
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-55 p-4 font-sans text-xs sm:text-sm text-text-main text-left animate-fade-in">
      <div className="relative bg-card-bg rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden border border-border-main animate-zoom-in">
        {/* Top Close Control */}
        ...
        <div className="p-6 text-center">
          ...
          <p className="text-[11px] text-slate-500 font-medium mt-3 leading-relaxed px-2">
            Are you sure you want to permanently delete the fine totaling{" "}
          ...
          <div className="mt-5 p-3.5 bg-rose-50 rounded-xl border border-rose-100 flex items-start gap-2.5 text-left">
            ...
            <p className="text-[11px] text-rose-700 font-bold leading-normal uppercase tracking-wide">
              Warning: This action will permanently remove this fine record from
              the history log.
            </p>
          </div>
        </div>
        ...
        {/* Footer Actions */}
        <div className="px-6 py-4 bg-slate-50 border-t border-border-main flex gap-2">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-2.5 text-xs font-bold uppercase tracking-wider text-slate-500 hover:text-text-main bg-card-bg border border-border-main rounded-xl transition-all cursor-pointer"
          >
            Abort
          </button>
          <button
