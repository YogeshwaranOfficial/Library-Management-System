...
export const RestoreFineModal = ({ isOpen, onClose, onConfirm, fine }: RestoreFineModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4 font-sans text-xs sm:text-sm text-slate-700 text-left animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 border border-slate-200 animate-zoom-in">
        <div className="text-center">
          ...
          <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wide">Restore Fine Record</h3>
          <p className="text-[11px] text-slate-500 font-medium mt-2.5 leading-relaxed">
            Are you sure you want to revert the payment transaction for <span className="font-bold text-slate-900">"{fine.memberName}"</span>? This will shift the record back to the active overdue list.
          </p>
        </div>
        ...
        <div className="mt-5 pt-1 flex gap-3">
          <button 
            type="button"
            onClick={onClose} 
            className="flex-1 py-2.5 text-xs font-bold uppercase tracking-wider bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-slate-900 border border-slate-200 rounded-xl transition-colors cursor-pointer"
          >
            Cancel
          </button>
          <button 
