...
export const SettleFinePaymentModal = ({
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4 font-sans text-xs sm:text-sm text-text-main text-left animate-fade-in">
      <div className="bg-card-bg rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-border-main animate-zoom-in">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="p-4 bg-slate-50 border border-border-main rounded-xl space-y-2 text-xs">
            <div className="flex justify-between font-bold text-slate-400 uppercase text-[11px] tracking-wide">
              <span>Account Name:</span>
              <span className="text-text-main font-sans font-bold normal-case tracking-normal text-xs">
                {fine.memberName}
              </span>
            </div>
                ...
                <button
                  ...
                  className={`py-2 text-xs font-bold uppercase tracking-wider border rounded-xl transition-all cursor-pointer ${
                    paymentMethod === method
                      ...
                      : "bg-slate-50 border-border-main text-slate-500 hover:text-text-main"
                  }`}
                >
                  {method}
                </button>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-border-main rounded-xl text-xs font-medium text-text-main placeholder:text-slate-400 outline-hidden focus:bg-card-bg focus:border-slate-900 transition-all cursor-pointer font-mono"
              />
              <Calendar
                size={14}
                className="text-slate-400 absolute left-3.5 top-3.5 pointer-events-none"
              />
            </div>
