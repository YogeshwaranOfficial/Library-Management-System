  ...
  return (
    <>
      ...
      <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4 font-sans text-xs sm:text-sm text-text-main text-left">
        <div className="bg-card-bg rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-slate-light/10 animate-zoom-in">
          {/* Header Block Panel */}
          <div className="bg-slate-900 p-5 text-white flex justify-between items-center border-b border-slate-light/10">
            <div>
              ...
              <span className="text-[11px] bg-card-bg/10  text-slate-400 font-mono font-bold py-0.5 rounded-md tracking-wide uppercase">
                {formattedIssueId}
              </span>
            </div>
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-border-main space-y-1.5">
              ...
              <div className="pt-2.5 mt-2 border-t border-border-main flex items-center gap-1.5">
                <span className="text-xs font-medium text-text-main">
                  Active Resource Holdings:{" "}
                  {isLoadingStats ? (
                    <span className="text-[11px] text-slate-400 animate-pulse font-bold tracking-wider uppercase">
                      Calculating...
                    </span>
                  ) : (
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-b border-border-main py-3 font-mono text-xs bg-slate-50/50 px-2 rounded-xl">
              <div>
                <div className="flex items-center gap-1 mb-1">
                  <Calendar size={13} className="text-slate-400" />
                  <span className="text-[11px] font-bold text-slate-400 uppercase block font-sans tracking-wide">
                    Checkout Signature
                  </span>
                </div>
                <div className="text-slate-800 font-bold pl-4">
                  {record.borrowedDate}
                </div>
              </div>
              <div>
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <span className="text-[11px] font-bold text-slate-800 uppercase block font-sans tracking-wide">
                    Target Return Due
                  </span>
                </div>
                <div className="text-text-main font-bold pl-4">
                  {record.dueDate}
                </div>
              </div>
            </div>
            ...
            <div className="pt-2 flex justify-between items-center gap-3">
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold uppercase tracking-wider text-text-main bg-slate-50 hover:bg-slate-100 rounded-xl border border-border-main transition-all cursor-pointer"
              >
                <Edit2 size={12} /> Edit
              </button>
              <div className="flex gap-2 items-center">
                <button
                  type="button"
                  onClick={onClose}
                  className="px-3 py-2 text-xs font-bold uppercase tracking-wider text-slate-500 hover:text-text-main cursor-pointer transition-colors"
                >
                  Cancel
                </button>
                <button
          ...
          <div className="bg-card-bg w-full max-w-md rounded-2xl border border-rose-100 shadow-2xl overflow-hidden animate-zoom-in">
            ...
            <div className="p-6 space-y-4 text-xs leading-relaxed text-text-main font-medium">
              ...
              <p className="text-[11px] text-slate-500 italic bg-slate-50 p-3 rounded-xl border border-border-main leading-normal">
                ...
                cash registration counter desk before restoring book items back
