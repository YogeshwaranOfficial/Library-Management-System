...
export const TransactionModal = ({
  ...
  const fixedRecordExpiry = editingRecord
    ? editingRecord.membershipExpiryDate || editingRecord.expiryDate || ""
    : "";

  // ✨ Initial State Configuration
  const [memberSearch, setMemberSearch] = useState(
    editingRecord ? editingRecord.memberName : "",
  );
  ...
  const [bookSearch, setBookSearch] = useState(
    editingRecord ? editingRecord.bookTitle : "",
  );
  ...
  const [dueDate, setDueDate] = useState(
    editingRecord ? editingRecord.dueDate : "",
  );
  ...
  useEffect(() => {
    if (selectedMember && isOpen) {
      axiosClient
        .get(`/issues/member-allowance/${selectedMember.member_id}`)
        .then((res) => {
          ...
          setBorrowMetrics({
            ...
            isExpired:
              ...
              selectedMember.compliance?.status === "EXPIRED",
  ...
  const isSelectedMemberMatchingInput =
    selectedMember && selectedMember.name === memberSearch;

  // Ensure info card stays loaded instantly on edit parameters
  const showMemberInfoCard =
    !!editingRecord || (hasMemberText && isSelectedMemberMatchingInput);

  const reachedPlanCap =
  ...
  const minAllowedDate = editingRecord
    ? formatToISODate(editingRecord.borrowedDate)
    : getTomorrowString();

  const isDateRangeInvalid =
    !!(maxAllowedDate && cleanDueDate > maxAllowedDate) ||
    cleanDueDate < minAllowedDate;
                    ...
                    return (
                      <li
                        ...
                        onClick={() => {
                          if (isBlocked) {
                            toast.error(
                              m.compliance?.message ||
                                "Cannot select member profile due to active policy blocks.",
                            );
                            return;
                          }
                    ...
                    return (
                      <li
                        ...
                        onClick={() => {
                          if (outOfStock) {
                            toast.error(
                              "Asset unavailable: Zero matching items remain on library storage shelves.",
                            );
                            return;
                          }
