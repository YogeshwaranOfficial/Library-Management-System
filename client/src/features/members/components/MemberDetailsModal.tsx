        ...
        <div className="w-full max-w-xl rounded-2xl bg-card-bg shadow-xl transition-all overflow-hidden border border-amber-100 flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center justify-between border-b border-slate-100 p-5 bg-slate-900">
            ...
            <button
              ...
              className="text-slate-400 hover:text-text-main transition-colors text-base font-bold cursor-pointer p-1.5 hover:bg-slate-100 rounded-lg"
            >
              ✕
            </button>
          </div>
            ...
            {!showRenewalScreen ? (
              ...
              <div className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <span
                    className={`self-start sm:self-auto inline-flex items-center px-3 py-1 rounded-full text-xs font-bold tracking-wide ${
                      member.isActive
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-6 text-sm">
                  <div>
                    ...
                    <span className="font-semibold text-text-main mt-1 block select-all text-base">
                      {member.email}
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="font-semibold text-text-main mt-1 block select-all text-base">
                      {member.phoneNumber || "No Verified Phone"}
                    </span>
                  </div>
                  <div>
                    <span className="block text-xs font-bold text-slate-400 uppercase tracking-wide">
                    ...
                    <div className="mt-1.5">
                      <span className="inline-block px-3 py-1.5 rounded-lg text-xs font-bold bg-slate-100 text-text-main border border-border-main/40">
                  ...
                  <div>
                    ...
                    <span className="font-bold text-text-main mt-1 block text-base">
                      {member.expiryDate}
                    </span>
                  </div>
                </div>
              ...
              <form onSubmit={handleRenewSubmit} className="space-y-6">
                <div className="p-4 bg-amber-50/60 border border-amber-200/60 rounded-xl text-sm text-slate-600 font-medium leading-relaxed">
                  You are renewing the subscription plan for{" "}
                  <span className="text-text-main font-bold">
                    {member.name}
                  </span>
                  . Submitting this update logs the start date as{" "}
                  <span className="text-text-main font-bold">Today</span> and
                  automatically assigns the corresponding contract validation
                  cycles.
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 bg-slate-50 border border-border-main text-slate-800 rounded-xl text-sm font-semibold outline-none cursor-pointer focus:bg-card-bg focus:border-slate-400 transition-all"
                  >
                    ...
                    {plans.map((p: MembershipPlan & { plan_name?: string }) => {
                      ...
                      const fallbackId =
                        planData.membership_plan_id || planData.id;
                      const fallbackName =
                        planData.plan_name || planData.name || "Unnamed Tier";
