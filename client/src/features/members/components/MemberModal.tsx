...
export const MemberModal = ({
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4 font-sans">
      <div className="bg-card-bg rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-amber-100 animate-zoom-in">
        {/* Modal Branding Header - Clean Light Structured Banner */}
        <div className="bg-slate-900 border-b border-slate-100 p-5 text-text-main flex justify-between items-center">
          <h3 className="text-xl text-white font-bold tracking-tight">
            {editingMember ? "Renew Membership Plan" : "Add New Member"}
          </h3>
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-text-main transition-colors cursor-pointer text-base font-bold p-1.5 hover:bg-slate-100 rounded-lg"
          >
            ✕
          </button>
        </div>
        ...
        <form
          ...
          className="p-6 space-y-5 text-text-main"
        >
          ...
          <div>
            ...
            <select
              ...
              className={`w-full px-4 py-3 border rounded-xl text-sm font-semibold outline-none transition-all ${
                hasNoAvailableUsers && !editingMember
                  ...
                  : "bg-slate-50 border-border-main text-slate-800 focus:bg-card-bg focus:border-slate-400"
            ...
            {errors.userId && (
              <p className="text-xs text-rose-700 mt-1.5 font-bold">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                {...register("email")}
                readOnly
                className="w-full px-4 py-3 bg-slate-100/70 border border-border-main rounded-xl text-sm text-slate-600 font-semibold outline-none cursor-not-allowed select-all"
              />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wide block mb-1.5">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-3 bg-slate-50 border border-border-main text-slate-800 rounded-xl text-sm font-semibold outline-none transition-all focus:bg-card-bg focus:border-slate-400 cursor-pointer"
            >
            ...
            <div className="flex items-center justify-between p-4 bg-amber-50/40 border border-amber-200/60 rounded-xl">
              <div className="pr-4">
                <span className="text-sm font-bold text-slate-800 block">
                  Membership Continuity Toggle
                </span>
                <span className="text-xs text-slate-400 font-medium mt-0.5 block">
                  Updating values shifts account validation cycles to today's
                  parameters.
                </span>
              </div>
              <input
                type="checkbox"
                {...register("isActive")}
                className="w-5 h-5 text-text-main border-slate-300 bg-card-bg rounded-lg focus:ring-0 cursor-pointer accent-slate-900 transition-all"
              />
            </div>
          )}
