...
export const MembersPage = () => {
    ...
    queryFn: async () => {
      ...
      const transformed = Array.isArray(rawRecords)
        ? rawRecords.map((dbRow: unknown): LibraryMember => {
            ...
            const planObj = (row.membership_plan || {}) as Record<
              string,
              unknown
            >;
            ...
            return {
              ...
              phoneNumber: String(
                userObj.phone_number || row.phoneNumber || "",
              ),
              membershipPlanId: String(
                row.membership_plan_id || row.membershipPlanId || "",
              ),
              membershipPlanName: String(
                planObj.plan_name || "No Plan Tier Assigned",
              ),
              activationDate: String(
                row.activation_date || row.activationDate || "",
              ),
  ...
  const saveMemberMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(
        selectedMember
          ? "Membership renewed cleanly."
          : "New library member created successfully!",
      );
      setIsFormOpen(false);
    ...
    onError: (error: unknown) => {
      let serverErrorMessage =
        "Database schema validation failed on submission updates.";
  ...
  return (
    <div className="flex flex-col min-h-screen relative animate-fade-in pb-12 font-sans text-xs sm:text-sm text-text-main text-left">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-card-bg p-5 mb-5 rounded-2xl border border-border-main shadow-xs shrink-0">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5 font-medium leading-relaxed">
            Click any record row to manage tier tracking, view info cards, or
            extend membership renewals.
          </p>
        </div>
        <button
          onClick={() => {
          ...
          className="flex items-center gap-1.5 px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold uppercase tracking-wider rounded-xl shadow-xs transition-all whitespace-nowrap cursor-pointer self-stretch sm:self-auto justify-center"
        >
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-border-main text-[11px] font-bold text-slate-400 uppercase bg-slate-50 tracking-wider">
                    ...
                    <th className="py-3.5 px-5">Current Plan</th>
                  ...
                  {memberList.length === 0 ? (
                    ...
                    memberList.map((member) => (
                      ...
                      >
                        ...
                        <td className="py-3.5 px-5">
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg text-[11px] font-bold uppercase tracking-wider bg-slate-50 border border-slate-100 text-text-main">
      ...
      <MemberDetailsModal
        ...
        onRenew={(planId) =>
          selectedMember &&
          renewMutation.mutate({ memberId: selectedMember.id, planId })
        }
