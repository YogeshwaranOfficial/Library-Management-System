...
export const DeletePlanModal = ({
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-fade-in text-left">
      <div className="bg-card-bg rounded-2xl shadow-xl w-full max-w-sm overflow-hidden border border-amber-100/80 animate-zoom-in relative">
        {/* Absolute Positioning Dismiss Controller */}
        <div className="absolute right-4 top-4">
          <button
            type="button"
            onClick={onClose}
            className="text-slate-400 hover:text-text-main transition-colors p-1 hover:bg-slate-50 rounded-lg cursor-pointer"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="p-6 text-center">
          ...
          <p className="text-xs text-slate-500 font-medium mt-3 leading-relaxed px-2">
            ...
            configuration plan{" "}
            <span className="font-bold text-text-main">"{planName}"</span> from
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex gap-3 text-xs font-bold tracking-wide">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-3 text-text-main bg-card-bg border border-border-main rounded-xl transition-all hover:bg-slate-100 cursor-pointer"
          >
            Abort
          </button>
          <button
