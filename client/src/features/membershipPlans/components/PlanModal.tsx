...
export const PlanModal = ({ isOpen, mode, plan, onClose }: PlanModalProps) => {
  ...
  const planMutation = useMutation({
    ...
    onError: (err: unknown) => {
      let errorMessage =
        "Execution error rejected parameter payload metrics map.";
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-fade-in text-left">
      <div className="bg-card-bg rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-amber-100/80 animate-zoom-in">
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-900 text-white">
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-1.5 text-slate-400 hover:text-text-main hover:bg-slate-100 rounded-lg transition-colors cursor-pointer"
          >
            <X size={16} />
          </button>
        </div>

        <form onSubmit={handleSubmit} className="p-6 space-y-5 text-text-main">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-border-main rounded-xl text-base font-semibold text-text-main placeholder:text-slate-400 outline-none focus:bg-card-bg focus:border-slate-400 transition-all"
              />
              <Layers
                size={16}
                className="text-slate-400 absolute left-4 top-3.5"
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-border-main rounded-xl text-base font-bold text-text-main placeholder:text-slate-400 outline-none focus:bg-card-bg focus:border-slate-400 transition-all"
              />
              <IndianRupee
                size={16}
                className="text-slate-400 absolute left-4 top-3.5"
          ...
          <div className="pt-5 border-t border-slate-100 flex gap-3 text-xs font-bold tracking-wide">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-3 bg-slate-50 border border-border-main text-text-main rounded-xl transition-all hover:bg-slate-100 cursor-pointer"
            >
              Cancel
            </button>
            <button
            ...
            >
              <Save size={15} />{" "}
              {planMutation.isPending ? "Syncing..." : "Confirm"}
            </button>
          </div>
        </form>
      </div>
