...
export const ReturnedDetailsModal = ({
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4 font-sans text-xs sm:text-sm text-slate-700 text-left animate-fade-in">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-slate-200 animate-zoom-in">
        
        {/* Header Block */}
        <div className="bg-slate-900 px-6 py-5 text-white flex justify-between items-center">
          <div>
            <h3 className="font-bold text-xs uppercase tracking-wider text-slate-200">Returned Book Details</h3>
            <span className="text-[11px] text-slate-400 font-mono tracking-widest">{formattedIssueId}</span>
          </div>
          <button 
            type="button"
            onClick={onClose} 
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-1.5">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wide block">Borrower Profile</span>
            <div className="text-sm font-bold text-slate-900">{record.memberName}</div>
            <div className="text-xs text-slate-500 font-medium flex items-center gap-1.5">
              <Mail size={12} className="shrink-0 text-slate-400" /> {record.memberEmail || "No email provided"}
            </div>
            <div className="text-xs text-slate-500 font-medium flex items-center gap-1.5">
              <Phone size={12} className="shrink-0 text-slate-400" /> {record.memberPhone || "No contact info"}
            </div>
          </div>

          {/* Book Card */}
          <div className="space-y-1.5 px-1">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wide block">Book Details</span>
            ...
            <div className="text-xs text-slate-500 font-medium pl-5.5">
              Author: <span className="font-semibold text-slate-700">{record.bookAuthor || "Unknown author"}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 border-t border-b border-slate-100 py-3.5 font-mono text-center text-[11px]">
            <div>
              <span className="text-[11px] font-bold text-slate-400 uppercase block font-sans tracking-wide mb-0.5">Issued</span>
              <div className="text-slate-700 font-bold">{record.borrowedDate}</div>
            </div>
            <div>
              <span className="text-[11px] font-bold text-slate-400 uppercase block font-sans tracking-wide mb-0.5">Due Date</span>
              <div className="text-slate-700 font-bold">{record.dueDate}</div>
            </div>
            <div>
              <span className="text-[11px] font-bold text-emerald-700 uppercase block font-sans tracking-wide mb-0.5">Returned</span>
          ...
          <div className="pt-2 flex flex-col gap-3">
            ...
            <div className="flex justify-between items-center gap-3 pt-3 border-t border-slate-100">
              <button 
                type="button" 
                onClick={onClose} 
                className="px-4 py-2 text-xs font-bold uppercase tracking-wider text-slate-500 hover:text-slate-900 cursor-pointer transition-colors"
              >
                Close View
              </button>
              <button
